...
// TODO: sometime shows partial list?
	...
	showAlert: function(bugs, isWhitelisted, host) {
		if ( (bugs) && (bugs.length > 0) ) {
			...
			for (var i = 0; i < bugs.length; i++) {
				...
				var userSiteSelections = ghostery.prefs.getWhitelistedDatabase(host);
				if (!isWhitelisted && ghostery.prefs.shouldBlockBug(bugs[i].bug.aid, userSiteSelections)) {
	...
	recordPageInfo: function(domain, pageLatency, adSpots) {
		var pageInfoSrc = 'http://l.ghostery.com/api/page/' + 
	...
	notificationBox: function() {
		...
		link.textContent = 'Ghostery\'s tracker list has been updated.';
